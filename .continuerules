...
> **Regenerated from `.anatomy` at commit `f084c88` by `anatomy-cli@1.0.0`.**
> DO NOT EDIT — changes will be overwritten on next `anatomy render`.
> Edit `.anatomy` instead, then run `anatomy render`.
> If your HEAD ≠ `f084c88`, this file may be stale — re-run `anatomy render`.
...
## Project structure
- `anatomy-cli/` — @anatomytool/cli — validate, generate (Pass 1 + optional Pass 2 AI fill), show, migrate, rehash, memory, mcp
- `anatomy-validate/` — @anatomytool/validate — version-routed JSON-schema validator, fingerprint/hash/path checks, cascading tree discovery
