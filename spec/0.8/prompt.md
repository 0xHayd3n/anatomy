# Anatomy Generation Prompt Template (v0.8)

**Status:** Normative for `.anatomy` v0.8 generation.
...
The v0.8 prompt differs from v0.7 in three places:
- `[code_profile]` is **removed**. The Pass 1 generator that produced this section was already removed in commit `ec73e00`; v0.8 finishes the cleanup by dropping the schema slot. Component/command/endpoint/export samples are derivable from source on every read.
- `[[substance.capabilities]]` is **removed**. "What the repo can do" is reconstructible from reading the code and README. Where a capability encodes a deliberate design choice, surface it as a `[[decisions]]` entry instead.
- `[[substance.limitations]]` is **removed**. Same reasoning: derivable from code/tests, or belongs in `[[decisions]]` as a chosen non-goal.

`[substance.key_dependencies]`, `[[rules]]`, `[[flows]]`, `[[decisions]]`, and the rest of v0.7's surface are unchanged.
...
The repository's Stack (technology) has already been determined deterministically by static analysis and is provided in the REPOSITORY SUMMARY below. You are filling in the remaining identity fields plus optional operation/substance/structure/environment/interface/domain_model fields where evidence supports them, plus the high-value rules/flows/decisions fields that capture knowledge a reader cannot infer from source code alone.
...
HIGH-VALUE FIELDS (the v0.7 additions, retained in v0.8 — emit when evidence supports them):
...
- tagline, description, conventions values, commands values, key_dependencies[].why, structure.entries[].purpose, interface.*.summary, domain_model.entities[].summary, rules[].rule, rules[].why, flows[].summary, decisions[].topic, decisions[].reason all have explicit max-length caps documented in the schema; respect them.
...
This prompt template is published at `https://anatomy.dev/spec/0.8/prompt.md` and is part of the v0.8 schema contract. Any change to the prompt requires bumping `anatomy_version` to `0.9` (additive) or `1.0` (breaking).
...
- **Rules / flows / decisions are gated on the "uncapturable from source" bar** — this is the v0.7 reorientation, retained in v0.8. The model is explicitly told not to pad with derivable observations to hit a count, because that's what `[[insights]]` failed at in v0.6 and what `[code_profile]` and `substance.capabilities`/`limitations` failed at in v0.7.
