# Anatomy Generation Prompt Template (v0.10)

**Status:** Normative for `.anatomy` v0.10 generation.
...
The v0.10 prompt differs from v0.9 in one addition:
- `EXISTING_AGENTS_MD` optional input — **new**. If the target repository has an existing `AGENTS.md` at its root (and it is NOT anatomy-generated), its content (truncated to 3000 chars) is provided as additional context. See [`pass2-prompt-contract.md`](pass2-prompt-contract.md) for field-influence rules. The optional `[generate]` block in the schema is **never** populated by Pass 2 — it encodes per-repo human render preferences, not derivable content.

Identity, `[[rules]]`, `[[flows]]`, `[[decisions]]`, the optional `[operation]`, `[structure]`, `[environment]` sections, and the rest of v0.9's output surface are unchanged.
...
This prompt template is published at `https://anatomy.dev/spec/0.10/prompt.md` and is part of the v0.10 schema contract. Any change to the prompt requires bumping `anatomy_version` to `0.11` (additive) or `1.0` (breaking).
