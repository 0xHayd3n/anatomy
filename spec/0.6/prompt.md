# Anatomy Generation Prompt Template (v0.6)

**Status:** Normative for `.anatomy` v0.6 generation.
...
The repository's Stack (technology) has already been determined deterministically by static analysis and is provided in the REPOSITORY SUMMARY below. You are filling in the remaining identity fields plus optional operation/substance/structure/environment/interface/domain_model/code_profile/architecture fields where evidence supports them.
...
- insights: 3–8 repo-specific observations a reader could NOT infer from the tagline,
  directory listing, or dependency list alone. Each has:
    type: one of architecture | pattern | data-model | protocol | invariant | constraint
    name: lowercase hyphenated slug, max 40 chars
    summary: single-line description, max 200 chars
  Omit the field entirely if fewer than 3 genuinely notable insights exist.
  Do NOT restate obvious facts ("this is a TypeScript project", "has a plugin directory").
...
- architecture.invariants: 1-5 plain-English rules governing this codebase that would
  surprise an outside contributor. Draw ONLY from README "Non-goals", CONTRIBUTING files,
  or prominent inline comments. Omit entirely if no evidence is present.
...
rule = "..."    # optional — 1-5 entries; omit section entirely if no evidence

[[insights]]
type = "pattern"
name = "plugin-model"
summary = "Extensions ship as npm packages; core registry discovers and loads them at startup"

[[insights]]
type = "architecture"
name = "channel-abstraction"
summary = "All messaging channels implement one Channel interface — send, receive, presence"
...
This prompt template is published at `https://anatomy.dev/spec/0.6/prompt.md` and is part of the v0.6 schema contract. Any change to the prompt requires bumping `anatomy_version` to `0.7` (additive) or `1.0` (breaking).
