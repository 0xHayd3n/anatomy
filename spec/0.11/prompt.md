# Anatomy Generation Prompt Template (v0.11)

**Status:** Normative for `.anatomy` v0.11 generation.
...
The v0.11 prompt is identical to v0.10's. The v0.11 schema bump adds optional `[generate]` fields for cross-tool renderers (Cursor, Aider, Cline, Roo, Continue, Windsurf), but those are human-configured preferences set in `.anatomy` — Pass 2 must NOT emit them.

Identity, `[[rules]]`, `[[flows]]`, `[[decisions]]`, the optional `[operation]`, `[structure]`, `[environment]` sections, and the rest of v0.10's output surface are unchanged.
...
This prompt template is published at `https://anatomy.dev/spec/0.11/prompt.md` and is part of the v0.11 schema contract. Any change to the prompt requires bumping `anatomy_version` to `0.12` (additive) or `1.0` (breaking).
