# Anatomy Generation Prompt Template (v0.13)

**Status:** Normative for `.anatomy` v0.13 generation.
...
The v0.13 prompt is identical to v0.12. The v0.13 schema bump adds a 4th
[[rules]].verify variant: `kind = "semgrep"` with two sub-shapes — inline
`pattern` + `lang`, or a `rule_file` path to a Semgrep YAML rule. Pass 2 MUST
NOT emit verify clauses — they are author-written, like for v0.12.
