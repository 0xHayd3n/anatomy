# Anatomy v1.0 Pass 2 Prompt Contract

> Extends the [v0.14 contract](../0.14/pass2-prompt-contract.md). Only the additions and changes from v0.14 are listed here; everything else is inherited.
...
The `AiFillResponse` shape adds four new optional arrays in v1.0 ([[vocabulary]], [[invariants]], [[anti_patterns]], [[prerequisites]]); all other fields are unchanged from v0.9. Pass 2 returns `tagline`, `description`, identity fields, and the optional rules/flows/decisions/vocabulary/invariants/anti_patterns/prerequisites arrays.

## v1.0 additions

v1.0 adds four new optional top-level array sections — [[vocabulary]], [[invariants]], [[anti_patterns]], [[prerequisites]] — emitted by DEFAULT (not RICH MODE) when evidence supports them. Each follows the same "omit if no qualifying entries" gate as [[rules]]/[[flows]]/[[decisions]]. See spec/1.0/prompt.md for the per-section evidence rules.
