# Anatomy Generation Prompt Template (v0.14)

**Status:** Normative for `.anatomy` v0.14 generation.
...
The v0.14 base prompt is identical to v0.13. The v0.14 schema bump adds 4 optional top-level fields (`author`, `license`, `docs_url`, `repository_url`) and re-introduces optional `[substance]` with `key_dependencies` carrying an optional `version` field — these capture README-derivable identifying facts and pinned dependency versions for quick-reference value.

The v0.14 generation flow has TWO modes:
...
This prompt template is published at `https://anatomy.dev/spec/0.14/prompt.md` and is part of the v0.14 schema contract. Any change to the prompt requires bumping `anatomy_version` to `0.15` (additive) or `1.0` (breaking).
