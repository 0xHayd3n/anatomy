# Anatomy Generation Prompt Template (v0.12)

**Status:** Normative for `.anatomy` v0.12 generation.
...
The v0.12 prompt is identical to v0.11. The v0.12 schema bump adds an optional
[[rules]].verify field that declares how to machine-check the rule against
source. Pass 2 MUST NOT emit this field — verify clauses are author-written.

Identity, `[[rules]]`, `[[flows]]`, `[[decisions]]`, the optional `[operation]`, `[structure]`, `[environment]` sections, and the rest of v0.11's output surface are unchanged.
...
This prompt template is published at `https://anatomy.dev/spec/0.12/prompt.md` and is part of the v0.12 schema contract. Any change to the prompt requires bumping `anatomy_version` to `0.13` (additive) or `1.0` (breaking).
