...
import { renderContinueArtifact } from "../src/render/continue.js";
...
function minimalAnatomy() {
  const root = mkdtempSync(join(tmpdir(), "anat-continue-"));
...
describe("renderContinueArtifact", () => {
  it("emits .continuerules as the path", () => {
    const r = minimalAnatomy();
    const out = renderContinueArtifact(r, {});
    expect(out.path).toBe(".continuerules");
  });
