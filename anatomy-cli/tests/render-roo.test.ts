...
import { renderRooArtifact } from "../src/render/roo.js";
...
function minimalAnatomy() {
  const root = mkdtempSync(join(tmpdir(), "anat-roo-"));
...
describe("renderRooArtifact", () => {
  it("emits .roorules as the path", () => {
    const r = minimalAnatomy();
    const out = renderRooArtifact(r, {});
    expect(out.path).toBe(".roorules");
  });
