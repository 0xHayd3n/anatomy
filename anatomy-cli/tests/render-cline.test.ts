...
import { renderClineArtifact } from "../src/render/cline.js";
...
function minimalAnatomy() {
  const root = mkdtempSync(join(tmpdir(), "anat-cline-"));
...
describe("renderClineArtifact", () => {
  it("emits .clinerules as the path", () => {
    const r = minimalAnatomy();
    const out = renderClineArtifact(r, {});
    expect(out.path).toBe(".clinerules");
  });
