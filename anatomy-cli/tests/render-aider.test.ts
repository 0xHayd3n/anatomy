...
import { renderAiderArtifact } from "../src/render/aider.js";
...
function minimalAnatomy() {
  const root = mkdtempSync(join(tmpdir(), "anat-aider-"));
...
describe("renderAiderArtifact", () => {
  it("emits CONVENTIONS.md as the path", () => {
    const r = minimalAnatomy();
    const out = renderAiderArtifact(r, {});
    expect(out.path).toBe("CONVENTIONS.md");
  });
