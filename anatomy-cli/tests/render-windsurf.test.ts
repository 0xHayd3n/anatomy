...
import { renderWindsurfArtifact } from "../src/render/windsurf.js";
...
function minimalAnatomy() {
  const root = mkdtempSync(join(tmpdir(), "anat-windsurf-"));
...
describe("renderWindsurfArtifact", () => {
  it("emits .windsurfrules as the path", () => {
    const r = minimalAnatomy();
    const out = renderWindsurfArtifact(r, {});
    expect(out.path).toBe(".windsurfrules");
  });
